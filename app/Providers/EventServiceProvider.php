        ...
        Event::listen('Aacotroneo\Saml2\Events\Saml2LoginEvent', function ($event) {
            ...
            if(!empty($account)){
                if($account->status == 1){
                    ...
                    if(empty($userInfo)){
                        header("Location: ".env('APP_URL')."/login/not-allowed");
                        exit;
                    } else {
                        $successToken =  $account->createToken('userToken'.$account->id)->accessToken;
                        header("Location: ".env('APP_URL')."/login/".$successToken);
                    }
                } else {
                    header("Location: ".env('APP_URL')."/login/not-active");
                    exit;
                }
            } else {
               header("Location: ".env('APP_URL')."/login/not-exist");
            }
            exit();
        });
