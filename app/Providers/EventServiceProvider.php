            ...
            if(!empty($account)){
                if($account->status == 1){
                    ...
                    if(count($roleInfo)>0 && $roleInfo->general_permission == 0 && $userInfo->id != $userInfo->vp_emp_number ){
                        header("Location: https://adports.meritincentives.com/login/not-allowed");
                        exit;
                    } else {
                        $successToken =  $account->createToken('userToken'.$account->id)->accessToken;
                        header("Location: https://adports.meritincentives.com/login/".$successToken);
                    }
                } else {
                    header("Location: https://adports.meritincentives.com/login/not-active");
                    exit;
                }
            } else {
                ...
                //     $expectedReturnTo = 'https://adports.meritincentives.com/login/error';
                ...
                // }
                header("Location: https://adports.meritincentives.com/login/not-exist");
            }
            exit();
        });

        Event::listen('Aacotroneo\Saml2\Events\Saml2LogoutEvent', function ($event) {
           header("Location: https://adports.meritincentives.com/login/");
           exit();
            // Auth::logout();
            // Session::save();
        });
