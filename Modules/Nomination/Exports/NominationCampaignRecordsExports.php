 ...
 */
class NominationCampaignRecordsExports implements WithHeadings, WithMapping ,FromCollection
{
	protected $param;
	...
	public function map($records) :array
    {
		...
		$nominated_user = ucfirst($records->nominated_user_first_name) ." " .ucfirst($records->nominated_user_last_name);
        $email 			= $records->nominated_user_email;
		$nominator_name = ucfirst($records->nominated_by_user_first_name) ." " .ucfirst($records->nominated_by_user_last_name);
        $nominator_email = $records->nominated_by_user_email;
		...
		try{
			...
			/*
            return [
                'Nomination Name' => ucfirst($records->nominated_user_first_name) ." " .ucfirst($records->nominated_user_last_name),
                'Nomination Email' => $records->nominated_user_email,
				'Nominator Name' => ucfirst($records->nominated_by_user_first_name) ." " .ucfirst($records->nominated_by_user_last_name),
                'Nominator Email' => $records->nominated_by_user_email,
                'Points' => (isset($records->points) && !empty($records->points)) ? $records->points : "-",
                'Status' => $status,
                'Date' => (new Carbon($records->created_at))->format('M d, Y H:i a'),
            ];
    ...
    {
        return ['Nomination Name', 'Nomination Email', 'Nominator Name', 'Nominator Email', 'Points', 'Status', 'Date'];
    }
